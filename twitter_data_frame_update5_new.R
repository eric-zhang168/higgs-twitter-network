...
{ 
  ...
  for (j in 1:24)
  {
    ...
    WF_temp<-WF[WF$time<=(start_time+3600*j),]
    nfr[(i-1)*24+j]<-length(unique(WF_temp$V1))
    fm[(i-1)*24+j]<-length(WM$time[WM$time<=(start_time+3600*j)])
    }
}

record_data<-data.frame(vertex=data_index,indegree=indegrees,states=s,times_of_friends_retweet=fr,no_of_friends_who_retweet=nfr,no_of_friends_mention=fm,centralities=b,time_in_data_frame=time_in_hour)
