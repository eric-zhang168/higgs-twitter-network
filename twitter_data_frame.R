...
for (i in 1:456626)
{
  ...
  b[((i-1)*16+1):(16*i)]<-betweenness(n4,i+1,directed=TRUE)
}
...
{ 
  ...
  for (j in 1:16)
  {
    if (nrows(W2[(w2$time<=(1341414000+j*3600))&(W2$V1==i),])!=0)
    {
  ...
  for (j in 1:16)
  {
    fr[(i-1)*16+j]<-nrows(W2[((W2$V1+1) %in% friends_of_that_person)&(W2$time<=(1341414000+3600*j)),])
    fm[(i-1)*16+j]<-nrows(mention_record[(mention_record$V2+1 %in% friends_of_that_person)&(mention_record$time<=(1341414000+3600*j)),])
  }
}
